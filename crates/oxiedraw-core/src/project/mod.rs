...
mod tests {
    ...
    fn round_trip() {
        ...
        save::save(&mut canvas, &props, &crate::components::ComponentLibrary::new(), &crate::text::fonts::FontRegistry::new(), None, 0.0, None, &path)
            .expect("save");
        let project = load::load(&path).expect("load");
    ...
    fn canvas_size_mismatch() {
        ...
        save::save(&mut canvas_a, &props, &crate::components::ComponentLibrary::new(), &crate::text::fonts::FontRegistry::new(), None, 0.0, None, &path)
            .expect("save");
        let project = load::load(&path).expect("load");
